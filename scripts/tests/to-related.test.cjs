...
test('1. the canonical corpus is unchanged by this phase', () => {
  ...
  // PIN MOVED 9754062f -> 9f093eee by Expansion v2: SAM.gov activation, plus
  ...
  assert.strictEqual(fp('data/tender-opportunities/opportunities.json'), '9f093eee');
...
mutate('M24 the canonical corpus fingerprint changes', () => {
  ...
  assert.strictEqual(fp, '9f093eee');
});
