...
test('every rating this wave carries was measured on the record’s own domain', () => {
  ...
  // every record published on it repeats that reading verbatim. A chamber that
  // publishes several rolls on one host must therefore report one figure, not
  // one per roll.
  ...
  assert.ok(BD.domainRatingProblems({
    ...
    metricsProvenance: {
      domainRating: {
        ...
        status: 'publicApiReading', measuredDomain: 'kirp.pl',
      },
    },
