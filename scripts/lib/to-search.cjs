...
(function (root, factory) {
  ...
  function scoreRecord(rec, q) {
    ...
    for (i = 0; i < q.terms.length; i += 1) {
      t = q.terms[i];
      // An exact code match is the strongest single signal available.
      if (rec.c && rec.c.indexOf(t) !== -1 && /^\d{2,10}$/.test(t)) score += WEIGHTS.code;
      if (rec.t && rec.t.indexOf(t) !== -1) score += WEIGHTS.title;
      if (rec.b && rec.b.indexOf(t) !== -1) score += WEIGHTS.buyer;
      if (rec.l && rec.l.indexOf(t) !== -1) score += WEIGHTS.classificationLabel;
      if (rec.g && rec.g.indexOf(t) !== -1) score += WEIGHTS.country;
      if (rec.d && rec.d.indexOf(t) !== -1) score += WEIGHTS.description;
    }
    return score;
  }
  ...
  function matchesFilters(rec, f) {
    ...
    if (f.scheme && (!rec.sch || rec.sch.indexOf(f.scheme) === -1)) return false;
  ...
  function comparator(sort, hasQuery) {
    ...
    if (sort === 'deadline') {
      return function (a, b) {
        ...
        var av = a.dl == null ? Infinity : a.dl;
        var bv = b.dl == null ? Infinity : b.dl;
        return av !== bv ? av - bv : byId(a, b);
      };
    }
    if (sort === 'published') {
      return function (a, b) {
